data:extend(
  {
    {
      type = "item",
      name = "fuel-can",
      icon = "__Darkstar2.0__/graphics/icons/fuel-can.png",
      icon_size = 32,
      rocket_launch_products = { { type = "item", name = "fuel-register", amount = 1 } },
      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "large-fuel-can",
      icon = "__Darkstar2.0__/graphics/icons/large-fuel-can.png",
      icon_size = 32,
      rocket_launch_products = { { type = "item", name = "large-fuel-register", amount = 1 } },
      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "fuel-register",
      icon = "__Darkstar2.0__/graphics/icons/fuel-register.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "large-fuel-register",
      icon = "__Darkstar2.0__/graphics/icons/large-fuel-register.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "xenon-thruster",
      icon = "__Darkstar2.0__/graphics/icons/xenon-thruster.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "xenon-tank",
      icon = "__Darkstar2.0__/graphics/icons/xenon-tank.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "xenon-thruster-tank",
      icon = "__Darkstar2.0__/graphics/icons/xenon-thruster-tank.png",
      icon_size = 32,
      rocket_launch_products = { { type = "item", name = "xenon-thruster-tank-register", amount = 1 } },
      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "xenon-thruster-tank-register",
      icon = "__Darkstar2.0__/graphics/icons/xenon-tank-thruster-register.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[fuel-can]",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-1",
      icon = "__Darkstar2.0__/graphics/icons/ssd-1.png",
      icon_size = 32,

      subgroup = "data",
      order = "1[ssd1]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-2",
      icon = "__Darkstar2.0__/graphics/icons/ssd-2.png",
      icon_size = 32,

      subgroup = "data",
      order = "2[ssd2]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-3",
      icon = "__Darkstar2.0__/graphics/icons/ssd-3.png",
      icon_size = 32,

      subgroup = "data",
      order = "3[ssd3]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-4",
      icon = "__Darkstar2.0__/graphics/icons/ssd-4.png",
      icon_size = 32,

      subgroup = "data",
      order = "4[ssd4]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-5",
      icon = "__Darkstar2.0__/graphics/icons/ssd-5.png",
      icon_size = 32,

      subgroup = "data",
      order = "5[ssd5]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-6",
      icon = "__Darkstar2.0__/graphics/icons/ssd-6.png",
      icon_size = 32,

      subgroup = "data",
      order = "6[ssd6]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-7",
      icon = "__Darkstar2.0__/graphics/icons/ssd-7.png",
      icon_size = 32,

      subgroup = "data",
      order = "7[ssd7]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-8",
      icon = "__Darkstar2.0__/graphics/icons/ssd-8.png",
      icon_size = 32,

      subgroup = "data",
      order = "8[ssd8]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-9",
      icon = "__Darkstar2.0__/graphics/icons/ssd-9.png",
      icon_size = 32,

      subgroup = "data",
      order = "9[ssd9]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "ssd-m",
      icon = "__Darkstar2.0__/graphics/icons/ssd-m.png",
      icon_size = 32,

      subgroup = "data",
      order = "m[ssdm]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-1",
      icon = "__Darkstar2.0__/graphics/icons/receiver-1.png",
      icon_size = 32,

      subgroup = "data",
      order = "001[receiver-1]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-2",
      icon = "__Darkstar2.0__/graphics/icons/receiver-2.png",
      icon_size = 32,

      subgroup = "data",
      order = "002[receiver-2]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-3",
      icon = "__Darkstar2.0__/graphics/icons/receiver-3.png",
      icon_size = 32,

      subgroup = "data",
      order = "003[receiver-3]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-4",
      icon = "__Darkstar2.0__/graphics/icons/receiver-4.png",
      icon_size = 32,

      subgroup = "data",
      order = "004[receiver-4]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-5",
      icon = "__Darkstar2.0__/graphics/icons/receiver-5.png",
      icon_size = 32,

      subgroup = "data",
      order = "005[receiver-5]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-6",
      icon = "__Darkstar2.0__/graphics/icons/receiver-6.png",
      icon_size = 32,

      subgroup = "data",
      order = "006[receiver-6]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-7",
      icon = "__Darkstar2.0__/graphics/icons/receiver-7.png",
      icon_size = 32,

      subgroup = "data",
      order = "007[receiver-7]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-8",
      icon = "__Darkstar2.0__/graphics/icons/receiver-8.png",
      icon_size = 32,

      subgroup = "data",
      order = "008[receiver-8]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-9",
      icon = "__Darkstar2.0__/graphics/icons/receiver-9.png",
      icon_size = 32,

      subgroup = "data",
      order = "009[receiver-9]",
      place_result = "",
      stack_size = 500
    },

    {
      type = "item",
      name = "receiver-m",
      icon = "__Darkstar2.0__/graphics/icons/receiver-m.png",
      icon_size = 32,

      subgroup = "data",
      order = "010[receiver-10]",
      place_result = "",
      stack_size = 500
    },

  }
)
